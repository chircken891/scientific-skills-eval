...
### D-08: CLAUDE.md 纳入上下文

检测到 GSD 项目后一并读取项目级 CLAUDE.md。提取编码规范、路径约定、项目约束，纳入 context signals 供路由参考。

### D-09: 实现形态 — 独立 helper 脚本

GSD 检测逻辑抽为 `gsd-context-detect.sh`，独立于 scientific-do/SKILL.md：
- 输入：启动目录路径
- 行为：向上遍历查找 `.planning/`（上限 5 层），解析 STATE.md / ROADMAP.md / PROJECT.md / CLAUDE.md
- 输出：JSON 到 stdout（跟 GSD CLI 模式一致）
- scientific-do Step 1 调用此脚本，捕获 JSON 内化为上下文

**Rationale:** 独立脚本可单独测试、CI 验证、供其他 skill 复用。

### D-10: 输出格式 — JSON stdout

采用 GSD CLI 一致模式：脚本输出结构化 JSON 到 stdout。scientific-do 捕获并解析后内化到内存上下文（非 env var），在后续路由和执行步骤中使用。
</decisions>
