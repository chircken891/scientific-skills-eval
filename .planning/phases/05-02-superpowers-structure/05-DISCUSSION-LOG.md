...
| Option | Description | Selected |
|--------|-------------|----------|
| 意图解析→skill路由 | 解析用户输入，决定调用哪个skill | |
| 依赖链编排 | 按顺序协调多个skill执行 | |
| 全部功能 | 意图解析 + skill路由 + 依赖链编排 + 冲突处理 | ✓ |
...
| Option | Description | Selected |
|--------|-------------|----------|
| 精确优先 | 精确匹配优先，未命中再模糊匹配 | |
| 模糊优先 | 优先模糊匹配，可被精确覆盖 | |
| 智能调优 | 精确匹配 + 模糊fallback + 智能调优 | ✓ |
...
| Option | Description | Selected |
|--------|-------------|----------|
| 核心优先 | 核心skill优先，替换方案辅助 | |
| 场景优先 | 根据上下文选择最合适的 | |
| 综合决策 | 上下文 + 用户偏好 + 历史使用习惯 | ✓ |
...
| Option | Description | Selected |
|--------|-------------|----------|
| 无状态 | skill之间不传递数据，各自独立 | |
| 管道传递 | skill执行后结果传给下一个skill | |
| 全局状态 | 维护全局状态，所有skill可读写 | ✓ |
| 混合模式 | 根据skill类型决定 | |
